// Builds the condensed "10km · Cheapest · Under $2.00 · Only Shell +2" summary
// shown next to the collapsed Advanced Search trigger button on the Fuel
// Prices and EV Charging pages.
export function buildAdvancedSearchSummary({
  sortOptions,
  sortBy,
  radius,
  includeBrands = [],
  ...
  if (maxPrice != null) parts.push(`Under $${maxPrice.toFixed(2)}`);
  ...
  if (accessTypes.length > 0) {
    parts.push(accessTypes.length === 1 ? accessTypes[0] : `${accessTypes[0]} +${accessTypes.length - 1}`);
  }
  return parts.join(' · ');
}
